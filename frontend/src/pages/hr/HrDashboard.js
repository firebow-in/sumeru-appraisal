...
const HrDashboard = () => {
  ...
  const leaveRequests = [{ id: 1, name: 'Mark Johnson', reason: 'Sick', type: 'leave' }];
          ...
          <>
            ...
            <div className="dashboard-grid">
              ...
              <div className="attendance-overview">
                              ...
                              <div className="time-info">
                                ...
                                <div className="time-values-column">
                                  {employee.checkIns.map((time, idx) => (
                                    <span key={`in-${employee.id}-${idx}`} className="time-value">
                                      {time}
                                    </span>
                                  ))}
                                </div>
                              ...
                              <div className="time-info">
                                ...
                                <div className="time-values-column">
                                  {employee.checkOuts.map((time, idx) => (
                                    <span
                                      key={`out-${employee.id}-${idx}`}
                                      className={`time-value ${time === 'Pending' ? 'pending' : ''}`}
                                    >
                                      {time}
                                    </span>
                                  ))}
                ...
                <button className="view-full-report-btn" onClick={() => setIsAttendanceModalOpen(true)}>
                  View Full Report
                </button>
              </div>
                              ...
                              <div className="time-info">
                                ...
                                <div className="time-values-column">
                                  {employee.checkOuts.map((t, idx) => (
                                    <span
                                      key={`out-${employee.id}-${idx}`}
                                      className={`time-value ${t === 'Pending' ? 'pending' : ''}`}
                                    >
                                      {t}
                                    </span>
                                  ))}
                    ...
                    {attendanceCorrectionRequestsAll.map((item) => {
                      ...
                      return (
                        <div
                          ...
                          tabIndex={0}
                          onKeyDown={(e) => {
                            if (e.key === 'Enter' || e.key === ' ') setExpandedCorrectionId(isOpen ? null : item.id);
                          }}
                        >
                            ...
                            {isOpen && (
                              <div className="correction-detail">
                                <div className="detail-line">
                                  Request manual {item.reason.toLowerCase().includes('checkin') ? 'check-in' : 'checkout'} for <strong>{item.requestedTime}</strong>.
                                </div>
